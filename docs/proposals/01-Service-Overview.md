...
| :---------------------------- | :---------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **High Availability (99.9%)** | ระบบต้องพร้อมใช้งาน 24/7 หยุดชะงักไม่ได้                          | Lambda + DynamoDB + S3 + EventBridge ทั้งหมดเป็น AWS Managed Services มี built-in HA หลาย AZ                                                                                                                                                                                                               |
...
| **Resilience & Idempotency**  | รองรับ Network หลุดชั่วคราว, Client Retry ได้โดยไม่เกิด Duplicate | Client-side: เก็บ Pending Actions ใน localStorage, Retry ด้วย Idempotency Key เดิม — Server-side: Lambda ตรวจสอบ Idempotency Key ใน DynamoDB ด้วย `attribute_not_exists(PK)` Condition → ป้องกัน Duplicate แบบ Atomic — Event-level: EventBridge Publish ล้มเหลว → Outbox Pattern → outbox-processor retry |
| **Data Durability**           | ข้อมูลต้องไม่สูญหาย                                               | DynamoDB 99.99% durability, S3 99.99% durability, EventBridge + Outbox Pattern ป้องกัน Event loss                                                                                                                                                                                                          |
